...
describe "RightApi::Client volume_types" do
  before(:all) do
    ...
    @resources = @client.clouds(:id => @cloud_id).show.volume_types
    @resource = @client.clouds(:id => @cloud_id).show.volume_types(:id => @volume_type_id)
  ...
  it "should return the correct methods for @resource.show.api_methods" do
    ...
    @resource_detail.api_methods.sort.should == [cloud, :created_at, :description, :href, :links, :name, :resource_uid, :show, :size, :updated_at].sort
  end
