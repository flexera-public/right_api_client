# > cd detail_specs
# > bundle exec spec volume_snapshot_spec.rb 

require File.join(File.dirname(__FILE__), 'get_ids')

describe "RightApi::Client volume_snapshots" do
  before(:all) do
    @client, @cloud_id, @volume_snapshot_id, @volume_id,  @volume_volume_snapshot_id = get_ids('volume_snapshots')
    
    @resources = @client.clouds(:id => @cloud_id).show.volume_snapshots
    @resource = @client.clouds(:id => @cloud_id).show.volume_snapshots(:id => @volume_volume_snapshot_id)
    @resource_detail = @resource.show
    
    @resources_volumes = @client.clouds(:id => @cloud_id).show.volumes(:id => @volume_id).show.volume_snapshots
    @resource_volumes = @client.clouds(:id => @cloud_id).show.volumes(:id => @volume_id).show.volume_snapshots(:id => @volume_volume_snapshot_id)
